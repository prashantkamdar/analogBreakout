...
function updateBall () {
    // player2.setPosition(potentVal, scene.screenHeight() - player2.height / 2)
    if (ball.y > ballMaxY) {
        info.changeLifeBy(-1)
        if (info.life() <= 0) {
...
function setXPosition () {
    potentVal = xtronModules.readPmData(ModuleIndex.Module1)
    player2.x = Math.map(potentVal, 0, 100, 0, scene.screenWidth())
}
...
function resetBall () {
    ...
    ball.setPosition(Math.map(potentVal, 0, 100, 0, scene.screenWidth()), scene.screenHeight() * 0.75)
    ballVx = 0
    ballVy = ballCurrSpeed - ballVx
    ball.setVelocity(ballVx, ballVy)
}
function buildSplashScreens () {
    headlines = [["Breakout! is", "(C) 1976 Atari Inc."], ["Programmed in", "MakeCode Arcade"], ["by", "Alex K."], ["Analog modification by ", "7thking"]]
